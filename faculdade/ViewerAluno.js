...
export default class ViewerAluno {
  ...
  constructor(ctrl) {
    ...
    // Mapeando abaixo as callbacks associadas a cada botão.
    // Observe que as callbacks não são métodos da classe Viewer. São
    // funções avulsas que estão no final deste arquivo.
  ...
  obterElemento(idElemento) {
    // document é um objeto da classe Document que pertence ao DOM (Document Object Model) e
    // está disponível em Javascript das páginas HTML.
    // O DOM descreve todas as classes que mapeiam os elementos presentes dentro
    ...
    // Em Javascript é possível acrescentar um atributo a um objeto a qualquer momento.
    // Assim, vamos aproveitar essa característica da linguagem para adicionar o atributo
    // 'viewer' no elemento que recuperamos com document.getElementById. Isso permitirá
  ...
  apresentar(pos, qtde, aluno) {
    ...
    if (aluno == null) {
      ...
      this.divAviso.innerHTML =
        "Posição: " + pos + " | Número de Alunos: " + qtde;
    }
  }
