...
export default class DaoAluno {
    ...
    let promessa = new Promise(function(resolve, reject) {
      ...
      consulta.onsuccess = function(evento) { 
        if(consulta.result != null) {
          // O retorno do IndexedDB nos devolve um objeto da classe Object que contém todos os 
          // atributos da classe Aluno. Entretanto, mesmo tendo todos os atributos de Aluno, ele 
          // não pertence à classe Aluno. Assim, vamos recuperar o objeto e gerar um objeto Aluno
          // com seus atributos.
          let obj = consulta.result;
          resolve(new Aluno(obj.matricula, obj.cpf, obj.nome, obj.email, obj.telefone)); 
        }        
        else
    ...
    let promessa = new Promise(function(resolve, reject) {
      ...
      indice.openCursor().onsuccess = function(event) {
        ...
        if (cursor) {  
          ...
          // crio o objeto Aluno e o incluo no array
          array.push(new Aluno(obj.matricula, obj.cpf, obj.nome, obj.email, obj.telefone));
  ...
  async incluir(aluno) {
    ...
    let resultado = new Promise( (resolve, reject) => {
      ...
      // Recuperando a representação JSON do objeto Aluno
      let strJSON = aluno.toJSON();
      // Como no indexedDB não podemos colocar o objeto Aluno por ter os seus atributos
      // com visibilidade privada, estamos gerando um objeto da classe Object com os 
      // mesmos atributos de Aluno, para o colocarmos no banco de dados. O método estático
      // JSON.parse pega uma String na representação JSON e gera um objeto da classe Object
      ...
      indice.openCursor(keyValue).onsuccess = function(evento) {
        ...
        if (cursor) {
          ...
          if (cursor.value.matricula == aluno.getMatricula()) {
            ...
            // JSON.parse pega uma String na representação JSON e gera um objeto da classe Object
            let obj = JSON.parse(strJSON);
            // Atualizo o banco utilizando o objeto da classe Object que geramos na instrução acima
